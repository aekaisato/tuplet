...
(syntax-spec
 ...
 (host-interface/definitions
  (track name:racket-var bpm:racket-expr measure:tup ...+)
  #:binding (export name)
...
(define-syntax compile-bind
  (syntax-parser
    [(_ (#%host-expression (op:prim arg ...)))
     (define/with-syntax fmt-op (format-symbol "rt:~a" #'op))
     #'(fmt-op arg ...)]
    [(_ (#%host-expression n:note)) #'n]
    [(_ (#%host-expression t:tup)) #'(compile-tuplet t)]
    [(_ (#%host-expression p:pat)) #'(compile-pattern p)]))